                            ...
                            size_t minrun)
{
    ...
    struct list_head *tmp = NULL, *tmp_iter = NULL;
    if (cmp(priv, list, next) > 0) {
        ...
        do {
            ...
            // do insert sort if this node does not follow ascending order
            // fix your fucking bug you loser
            if (cmp(priv, head, head->next) < 0) {
                tmp = head, tmp_iter = head;
                
                while(tmp_iter->next && cmp(priv, tmp_iter, tmp_iter->next) < 0) 
                    tmp_iter = tmp_iter->next;

                tmp->next = tmp_iter->next;
                tmp_iter->next = tmp;
